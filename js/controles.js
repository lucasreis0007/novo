...
// Despesa OU Resgate contam como despesa — ver dashboard.js/historico.js/
// relatorios.js, que usam a mesma regra) ----------------
...
function atualizarQuantoGastei() {
    ...
    if (gastosPeriodo.length === 0) {
        ...
        elMaior.innerHTML = `${moeda(Number(maior.valor))} · ${iconeCategoria(maior.categoria)} ${maior.categoria}`;
    }
    ...
    document.getElementById("gastoCategoriaTop").innerHTML =
        categoriasOrdenadas.length > 0
            ? `${iconeCategoria(categoriasOrdenadas[0][0])} ${categoriasOrdenadas[0][0]} (${moeda(categoriasOrdenadas[0][1])})`
            : "—";
    ...
    categoriasOrdenadas.forEach(([categoria, valor]) => {
        ...
        linha.innerHTML = `
            <div class="linha-categoria-topo">
                <span>${iconeCategoria(categoria)} ${categoria}</span>

...
// (Controles / Alertas inteligentes / Lembretes / Notificações)
//
...
// ---------------- RESUMO "CONTROLES" (versão enxuta no dashboard —
// o controle completo, com filtros de período e detalhamento por
// categoria, agora mora em controles.html/controles.js) ----------------
...
function iconeAlertaDashboard(tipo) {
    const icones = {
        estourado: ['#EF4444', '<path d="M12 3 2.8 20h18.4L12 3Z"/><path d="M12 9v5M12 17h.01"/>'],
        orcamento: ['#F59E0B', '<path d="M12 3a9 9 0 1 0 9 9"/><path d="M12 7v5l3 2"/>'],
        atrasado: ['#DC2626', '<circle cx="12" cy="12" r="8.5"/><path d="M12 8v4l2.5 2"/>'],
        lembrete: ['#3B82F6', '<path d="M6 10a6 6 0 0 1 12 0v4l2 3H4l2-3v-4Z"/><path d="M10 20h4"/>'],
        gastos: ['#6366F1', '<path d="m5 16 4-4 3 3 6-7"/><path d="M14 8h4v4"/>'],
        meta: ['#16A34A', '<circle cx="12" cy="12" r="8.5"/><circle cx="12" cy="12" r="4"/><path d="m15 9-3 3"/>']
    };
    const [cor, svg] = icones[tipo] || ['#64748B', '<rect x="4" y="4" width="16" height="16" rx="3"/><path d="M8 12h.01M12 12h.01M16 12h.01"/>'];
    return `<span class="icone-alerta" style="--alerta-cor:${cor}" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round">${svg}</svg></span>`;
}
...
function renderizarAlertasDashboard(infoLembretes) {
    ...
    alertas.forEach(alerta => {
        ...
        el.className = "alerta";
        el.innerHTML = `${iconeAlertaDashboard(alerta.icone)}<p>${alerta.texto}</p>`;

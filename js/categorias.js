import { protegerPagina, carregarDados, criarArmazenamento, sair, iconeCategoriaSVG, listaIconesCategoria } from "./utils.js";
...
const iconeSelecionadoPreview = document.getElementById("iconeSelecionadoPreview");

let iconeSelecionado = "outros";
...
function carregarIconesCategorias() {
    const icones = JSON.parse(localStorage.getItem("iconesCategorias"));
    if (
        !icones ||
        typeof icones !== "object" ||
        Array.isArray(icones)
    ) {
        return { entrada: {}, saida: {} };
    }

    return {
        entrada: icones.entrada && typeof icones.entrada === "object" ? icones.entrada : {},
        saida: icones.saida && typeof icones.saida === "object" ? icones.saida : {}
    };
}

function salvarIconesCategorias(icones) {
    localStorage.setItem("iconesCategorias", JSON.stringify(icones));
}

function atualizarPreviewIcone() {
    iconeSelecionadoPreview.innerHTML = iconeCategoriaSVG(
        "Outros",
        { outros: iconeSelecionado }
    );
    document.querySelectorAll(".icone-opcao").forEach(botao => {
        botao.classList.toggle("selecionado", botao.dataset.icone === iconeSelecionado);
    });
}

function renderizarSeletorIcones() {
    const icones = listaIconesCategoria();
    seletorIcones.innerHTML = "";

    Object.keys(icones).forEach(chave => {
        const botao = document.createElement("button");
        botao.type = "button";
        botao.className = "icone-opcao";
        botao.dataset.icone = chave;
        botao.title = chave;
        botao.innerHTML = iconeCategoriaSVG("Outros", { __icone: chave });

        botao.addEventListener("click", () => {
            iconeSelecionado = chave;
            atualizarPreviewIcone();
        });

        seletorIcones.appendChild(botao);
    });

    atualizarPreviewIcone();
}

function selecionarIconeDaCategoria(nome, tipo) {
    const icones = carregarIconesCategorias();
    const salvo = icones[tipo]?.[nome];

    iconeSelecionado = salvo || "outros";
    atualizarPreviewIcone();
}

function salvarIconeDaCategoria(nome, tipo, icone) {
    const icones = carregarIconesCategorias();

    if (!icones[tipo]) icones[tipo] = {};

    icones[tipo][nome] = icone;
    salvarIconesCategorias(icones);
}

function removerIconeDaCategoria(nome, tipo) {
    const icones = carregarIconesCategorias();

    if (icones[tipo]) {
        delete icones[tipo][nome];
    }
    ...
    lista.forEach((item, index) => {
        ...
        card.innerHTML = `
            <div class="categoria-nome">
                <strong>${iconeCategoriaSVG(nome, { [tipoAtivo]: carregarIconesCategorias()[tipoAtivo] })} ${nome}</strong>
...
function listaCategoriaNome(categorias, index) {
    const lista = tipoAtivo === "saida" ? categorias.saida : categorias.entrada;
    const item = lista[index];
    return tipoAtivo === "saida" ? item?.nome : item;
}
...
function excluirCategoria(index) {
    ...
    const lista = tipoAtivo === "saida" ? categorias.saida : categorias.entrada;
    const item = lista[index];
    const nomeExcluido = tipoAtivo === "saida" ? item?.nome : item;
    ...
    if (nomeExcluido) {
        removerIconeDaCategoria(nomeExcluido, tipoAtivo);
    }
...
function editarCategoria(index) {
    ...
    selecionarIconeDaCategoria(nomeInput.value, tipoAtivo);
...
formCategoria.addEventListener("submit", (e) => {
    ...
    const listaAtual = tipoAtivo === "saida" ? categorias.saida : categorias.entrada;
    const itemAnterior = editandoIndex !== null ? listaAtual[editandoIndex] : null;
    const nomeAnterior = tipoAtivo === "saida"
        ? itemAnterior?.nome
        : itemAnterior;
    ...
    // Ícone fica em uma estrutura separada para não alterar o formato
    // original das categorias nem afetar nenhuma lógica financeira.
    const icones = carregarIconesCategorias();

    if (nomeAnterior && nomeAnterior !== nome) {
        delete icones[tipoAtivo][nomeAnterior];
    }

    icones[tipoAtivo][nome] = iconeSelecionado;
    salvarIconesCategorias(icones);

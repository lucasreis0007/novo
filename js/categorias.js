import { protegerPagina, carregarDados, criarArmazenamento, sair, iconeCategoria, idIconeCategoria, ICONES_DISPONIVEIS, renderizarIcone } from "./utils.js";
...
let iconeEscolhidoManualmente = false;
...
function renderizarSeletorIcones() {
    ...
    seletorIcones.innerHTML = ICONES_DISPONIVEIS.map(icone => `
        <button type="button" class="opcao-icone" data-icone="${icone.id}" title="${icone.rotulo}">
            ${renderizarIcone(icone.id, 36)}
        </button>
    `).join("");

    seletorIcones.querySelectorAll(".opcao-icone").forEach(botao => {
        botao.addEventListener("click", () => {
            iconeEscolhidoManualmente = true;
            selecionarIcone(botao.dataset.icone);
        });
    });
}
...
function selecionarIcone(id) {
    ...
    seletorIcones.querySelectorAll(".opcao-icone").forEach(botao => {
        botao.classList.toggle("selecionado", botao.dataset.icone === id);
    });
}

// Enquanto o usuário ainda não clicou em nenhum ícone manualmente, a
// sugestão acompanha o que ele vai digitando (mesma lógica automática
// por palavra-chave de antes).
nomeInput.addEventListener("input", () => {

    if (iconeEscolhidoManualmente) return;

    selecionarIcone(idIconeCategoria(nomeInput.value));
});
    ...
    lista.forEach((item, index) => {
        ...
        card.innerHTML = `
            <div class="categoria-nome">
                <strong>${iconeCategoria(nome, categorias)} ${nome}</strong>
...
function excluirCategoria(index) {
    ...
    const categorias = carregarCategorias();
    const lista = tipoAtivo === "saida" ? categorias.saida : categorias.entrada;
    const nome = tipoAtivo === "saida" ? lista[index]?.nome : lista[index];
    ...
    // Remove o ícone personalizado dessa categoria, se tinha um salvo.
    if (nome && categorias.iconesPersonalizados) {
        delete categorias.iconesPersonalizados[nome];
    }
...
function editarCategoria(index) {
    ...
    iconeEscolhidoManualmente = true;
    selecionarIcone(idIconeCategoria(nome, categorias));
...
function cancelarEdicao() {
    ...
    iconeEscolhidoManualmente = false;
    selecionarIcone("outros");
...
formCategoria.addEventListener("submit", (e) => {
    ...
    if (!categorias.iconesPersonalizados) {
        categorias.iconesPersonalizados = {};
    }

    const nomeAntigo = editandoIndex !== null
        ? (tipoAtivo === "saida" ? categorias.saida[editandoIndex]?.nome : categorias.entrada[editandoIndex])
        : null;
    ...
    // Se o nome mudou durante uma edição, move o ícone salvo pra chave nova.
    if (nomeAntigo && nomeAntigo !== nome) {
        delete categorias.iconesPersonalizados[nomeAntigo];
    }

    categorias.iconesPersonalizados[nome] = iconeSelecionado;
